...
const PlayerPaymentList: React.FC<{
    ...
    <div>
        <h3 className="text-xl font-bold text-center text-primary mb-4">{title}</h3>
        <div className="bg-surface p-4 rounded-xl border border-secondary/20 shadow-md">
            <ul className="space-y-3">
                {players.map(player => (
                    <li key={player.id} className="flex items-center justify-between bg-background/50 p-3 rounded-lg shadow-md">
                        ...
                        <label className="flex items-center cursor-pointer">
                            ...
                            />
                            <div className={`w-11 h-6 rounded-full transition-colors ${playerPayments[player.id] ? 'bg-primary' : 'bg-surface'}`}>
...
const PaymentTracker: React.FC<PaymentTrackerProps> = ({
  ...
  matchInfo,
    ...
    return (
         <div className="bg-surface/75 p-6 md:p-8 rounded-xl shadow-2xl border border-secondary/20 animate-fade-in w-full max-w-4xl mx-auto space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold text-primary">Gestión de Pagos</h2>
                <button onClick={onBackToField} className="bg-surface hover:bg-surface/75 text-text-primary font-bold py-2 px-4 rounded-lg transition-colors flex items-center space-x-2">
            ...
            <div className="bg-background/50 p-4 rounded-lg text-center shadow-md">
                <p className="text-sm font-semibold text-text-secondary">Cuota por Jugador</p>
                <p className="text-2xl font-bold text-text-primary mt-1">{formatCurrency(feePerPlayer)}</p>
            </div>
            ...
            <div>
                <h3 className="text-xl font-bold text-center text-primary mb-4">Resumen de Pagos</h3>
                <div className="bg-background/50 p-6 rounded-lg space-y-4 shadow-md">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                        <div className="bg-surface p-3 rounded-lg shadow-md"><p className="text-sm text-text-secondary">Total Recaudado</p><p className="text-lg font-bold text-primary">{formatCurrency(totalCollected)}</p></div>
                        <div className="bg-surface p-3 rounded-lg shadow-md"><p className="text-sm text-text-secondary">Total Cancha</p><p className="text-lg font-bold text-text-primary">{formatCurrency(totalAmount)}</p></div>
                        <div className="bg-surface p-3 rounded-lg shadow-md"><p className="text-sm text-text-secondary">Faltan por Pagar</p><p className="text-lg font-bold text-warning">{remainingCount}</p></div>
                        <div className="bg-surface p-3 rounded-lg shadow-md"><p className="text-sm text-text-secondary">Monto Pendiente</p><p className="text-lg font-bold text-error">{formatCurrency(remainingAmount)}</p></div>
                    </div>
                </div>
            </div>

            <div className="border-t border-secondary/20 pt-6 mt-6 flex flex-col sm:flex-row items-center gap-4 justify-end">
                <input type="file" ref={importInputRef} onChange={handleFileImport} className="hidden" accept=".json,application/json" />
                <button onClick={() => importInputRef.current?.click()} className="w-full sm:w-auto bg-surface hover:bg-surface/75 text-text-primary font-bold py-2 px-4 rounded-lg transition-colors flex items-center justify-center space-x-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM6.293 6.707a1 1 0 010-1.414l3-3a1 1 0 011.414 0l3 3a1 1 0 01-1.414 1.414L11 5.414V13a1 1 0 11-2 0V5.414L7.707 6.707a1 1 0 01-1.414 0z" clipRule="evenodd" /></svg>
                    <span>Importar Lista</span>
                </button>
                <button onClick={handleShare} className="w-full sm:w-auto bg-secondary hover:bg-blue-400 text-white font-bold py-2 px-4 rounded-lg transition-colors flex items-center justify-center space-x-2">
